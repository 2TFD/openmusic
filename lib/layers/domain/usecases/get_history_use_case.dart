...
class GetHistoryUseCase {
  final ListeningSummaryRepository listeningSummaryRepository;
  final TrackRepository trackRepository;

  GetHistoryUseCase({
    required this.listeningSummaryRepository,
    required this.trackRepository,
  });

  Future<List<Track>> execute({int limit = 20}) async {
    final ids = await listeningSummaryRepository.getRecentTrackIds(
      limit: limit,
    );
